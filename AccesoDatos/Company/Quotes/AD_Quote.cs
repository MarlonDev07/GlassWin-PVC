...
namespace AccesoDatos.Company.Quotes
{
    public class AD_Quote
    {
        ...
        public DataTable LoadQuotesFacturadas()
        {
            ...
            {
                DataTable dataTable = new DataTable();
                string query = "select Q.IdClient,Q.IdQuote,Q.Date,C.Name,Q.ProjectName,Q.Address,Q.SubTotal,Q.IVA,Q.Total from Quote Q Inner Join Client C ON Q.IdClient = C.IdClient where C.IdCompany = @IdCompany and State = 'Factura' order by Q.IdQuote desc";
                SqlCommand cmd = new SqlCommand(query, Cnn.OpenConecction());
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@IdCompany", CompanyCache.IdCompany);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
